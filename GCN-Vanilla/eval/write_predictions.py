...
class Predictions:
    def __init__(self, graph, pred, initial_predictions, final_predictions, No_Rel=False, dominant_entity='F'):
        '''
        ...
        test_index = graph.test_ids
        shuffle_text = graph.shuffle_text
        label_list = graph.label_list
        # shuffle_doc_name_str = '\n'.join(shuffle_text)
        # f = open('debug_shuffle.txt', 'w')
        # f.write(shuffle_doc_name_str)
        # f.close()

        self.write_relations(test_index, shuffle_text, label_list)
        self.renumber_relations()

    def write_relations(self, test_index, shuffle_text, label_list):
        """
        ...
        for i in range(len(test_index)):
            ...
            pred_label = label_list[ self.pred_labels[i]]
            for x in range(len(shuffle_text)):
                ind = shuffle_text[x].split("\t")[0]

                if ind == str(test_ind):

                    file = shuffle_text[x].split("\t")[3].strip('][').split(', ')[0].strip("'")
                    e1 = shuffle_text[x].split("\t")[3].strip('][').split(', ')[1].strip("'")
                    e2 = shuffle_text[x].split("\t")[3].strip('][').split(', ')[2].strip("'")
                    if len(str(file)) == 1:
                        f = "000" + str(file) + ".ann"
                    elif len(str(file)) == 2:
                        f = "00" + str(file) + ".ann"
