...
class Set_Connection:
    def __init__(self, sentence_only = False, sentences=None, labels=None, preceding_segs=None, concept1_segs=None,
                 ...
                 rel_labels=None, no_labels=None, no_rel_multiple=False, CSV=True, test=False, parallelize= False, no_of_cores = 64, predictions_folder = None, write_Entites = False, down_sample=None, down_sample_ratio=0.2):
        """
        ...
        # data - segmented and in CSV format
        if self.CSV:
            ...
            if self.parallelize:
                self.data_object = Segmentation(self.dataset, self.rel_labels, self.no_labels, self.no_rel_multiple, test=self.test, parallelize = True, no_of_cores=no_of_cores, predictions_folder = predictions_folder, write_Entites = write_Entites, down_sample=down_sample, down_sample_ratio=down_sample_ratio ).segments
            else:
                self.data_object = Segmentation(self.dataset, self.rel_labels, self.no_labels, self.no_rel_multiple, test=self.test, predictions_folder = predictions_folder, write_Entites = write_Entites, down_sample=down_sample, down_sample_ratio=down_sample_ratio).segments
